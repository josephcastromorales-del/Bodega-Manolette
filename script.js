...
function render() {
    ...
    productos.forEach(p => {
        ...
        card.innerHTML = `
            ...
            <div class="actions">
                <button onclick="cambiarEstado('${p.id}','llegaron')">Nuevo</button>
                <button onclick="cambiarEstado('${p.id}','espera')">Espera</button>
                <button onclick="cambiarEstado('${p.id}','listos')">Listo</button>
                ${isOwner ? `<button class="danger" onclick="borrarProducto('${p.id}')">Borrar</button>` : ''}
            </div>
        `;
